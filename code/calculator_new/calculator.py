...
class Calculator:
    ...
    def mem(self, mode):
        if mode == "MC":
            ...
            self.calc()
            self.memory = float(self.value) + float(self.memory)
            if self.memory == int(self.memory): self.memory = str(int(self.memory))
        elif mode == "M-":
            self.calc()
            self.memory = float(self.value) - float(self.memory)
            if self.memory == int(self.memory): self.memory = str(int(self.memory))
    ...
    def check_value(self, item):
        ...
        if item in [
                "(", "√", "-", "1/", "[i]e[/i]", "π", "10^",
                "log(", "ln(", "abs(", "sin(", "cos(", "tan(", "sec(", "csc(", "cot(",
                *numbers
                ] and self.value == "0":
            return True
        ...
        elif item == "+" and self.value[-1] not in numbers + ["e", ")", "π" "E"]: return False
        elif item == "−" and self.value[-1] not in numbers + [")", "π" "E"]: return False
        elif item == "×" and self.value[-1] not in numbers + [")", "π" "E"]: return False
        elif item == "÷" and self.value[-1] not in numbers + [")", "π" "E"]: return False
        elif item == "-" and self.value[-1] not in ["e", "(", "+", "−", "×", "÷", "^", "%"]: return False
        elif item == "." and self.value[-1] not in numbers: return False
        elif item in numbers and (self.value[-1] in [")", "π" "E"]): return False
        elif item == "π" and self.value[-1] not in ["+", "−", "×", "÷", "√", "^", "%", "("]: return False
        elif item == "[i]e[/i]" and self.value[-1] not in ["+", "−", "×", "÷", "√", "^", "%", "("]: return False
    ...
    def add_value(self, item):
        if self.check_value(item):
            if self.value == "0" and item not in ["+", "−", "×", ".", "^", "÷", "!", "^", "%", "e"]: self.value = item
    ...
    def eval(self, exp):
        ...
        # Constants
        while "π" in operations: operations[operations.index("π")] = str(pi)
        while "E" in operations: operations[operations.index("E")] = str(e)
        ...
        if int(float(operations[0])) == float(operations[0]): operations[0] = str(int(float(operations[0])))
        operations[0] += ending
        return operations[0]
